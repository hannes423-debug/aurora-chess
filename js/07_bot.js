...
function ray(lm,piece,dx,dy,dz){const{x,y,z}=piece.userData;for(let i=1;i<8;i++){const tx=x+dx*i,ty=y+dy*i,tz=z+dz*i;if(tx<0||tx>7||ty<0||ty>7||tz<0||tz>=LAYERS)break;const t=occ(tx,ty,tz);if(!t){lm.push({x:tx,y:ty,z:tz});}else{if(t.userData.color!==piece.userData.color)lm.push({x:tx,y:ty,z:tz});break;}}}
function getKnightMoves(p){const moves=[],{x,y,z}=p.userData;[[2,1,0],[2,-1,0],[-2,1,0],[-2,-1,0],[2,0,1],[2,0,-1],[-2,0,1],[-2,0,-1],[1,2,0],[1,-2,0],[-1,2,0],[-1,-2,0],[0,2,1],[0,2,-1],[0,-2,1],[0,-2,-1],[1,0,2],[1,0,-2],[-1,0,2],[-1,0,-2],[0,1,2],[0,1,-2],[0,-1,2],[0,-1,-2]].forEach(function(d){const tx=x+d[0],ty=y+d[1],tz=z+d[2];if(tx<0||tx>7||ty<0||ty>7||tz<0||tz>=LAYERS)return;const t=occ(tx,ty,tz);if(!t||t.userData.color!==p.userData.color)moves.push({x:tx,y:ty,z:tz});});return moves;}
...
function getKingMoves(p){const moves=[],{x,y,z}=p.userData;for(let dx=-1;dx<=1;dx++){for(let dy=-1;dy<=1;dy++){if(dx===0&&dy===0)continue;const tx=x+dx,ty=y+dy;if(tx<0||tx>7||ty<0||ty>7)continue;const t=occ(tx,ty,z);if(!t||t.userData.color!==p.userData.color)moves.push({x:tx,y:ty,z:z});}}[[0,0,1],[0,0,-1]].forEach(function(d){const tz=z+d[2];if(tz<0||tz>=LAYERS)return;const t=occ(x,y,tz);if(!t||t.userData.color!==p.userData.color)moves.push({x:x,y:y,z:tz});});if(!p.userData.moved){const row=y,layer=z;let r=occ(7,row,layer);if(r&&r.userData.type==='rook'&&!r.userData.moved&&!occ(5,row,layer)&&!occ(6,row,layer))moves.push({x:6,y:row,z:layer,castle:'kingside'});r=occ(0,row,layer);if(r&&r.userData.type==='rook'&&!r.userData.moved&&!occ(1,row,layer)&&!occ(2,row,layer)&&!occ(3,row,layer))moves.push({x:2,y:row,z:layer,castle:'queenside'});}return moves;}
function getPawnMoves(p){const moves=[],dir=p.userData.color==='white'?1:-1,x=p.userData.x,y=p.userData.y,z=p.userData.z;const f1=y+dir,f2=y+dir*2;if(f1>=0&&f1<8&&!occ(x,f1,z))moves.push({x:x,y:f1,z:z});if(!p.userData.moved&&f2>=0&&f2<8&&!occ(x,f1,z)&&!occ(x,f2,z))moves.push({x:x,y:f2,z:z,doublePushY:true});const u1=z+dir,u2=z+dir*2;if(u1>=0&&u1<LAYERS&&!occ(x,y,u1))moves.push({x:x,y:y,z:u1});if(!p.userData.moved&&u2>=0&&u2<LAYERS&&!occ(x,y,u1)&&!occ(x,y,u2))moves.push({x:x,y:y,z:u2,doublePushZ:true});[[1,dir],[-1,dir]].forEach(function(d){const tx=x+d[0],ty=y+d[1];if(tx>=0&&tx<8&&ty>=0&&ty<8){const t=occ(tx,ty,z);if(t&&t.userData.color!==p.userData.color)moves.push({x:tx,y:ty,z:z});}});[[1,dir],[-1,dir]].forEach(function(d){const tx=x+d[0],tz=z+d[1];if(tx>=0&&tx<8&&tz>=0&&tz<LAYERS){const t=occ(tx,y,tz);if(t&&t.userData.color!==p.userData.color)moves.push({x:tx,y:y,z:tz});}});[1,-1].forEach(function(dx){const tx=x+dx,ty=y+dir,tz=z+dir;if(tx>=0&&tx<8&&ty>=0&&ty<8&&tz>=0&&tz<LAYERS){const t=occ(tx,ty,tz);if(t&&t.userData.color!==p.userData.color)moves.push({x:tx,y:ty,z:tz});}});if(lastDoublePawn&&lastDoublePawn.color!==p.userData.color){const ep=lastDoublePawn;if(ep.pz===z&&Math.abs(ep.px-x)===1){if(ep.doublePushY&&ep.py===y){const capY=ep.py+dir;if(capY>=0&&capY<8&&!occ(ep.px,capY,z))moves.push({x:ep.px,y:capY,z:z,enPassant:{px:ep.px,py:ep.py,pz:ep.pz}});}}if(ep.doublePushZ&&ep.px===x&&Math.abs(ep.pz-z)===1){const capZ=ep.pz-dir;if(capZ>=0&&capZ<LAYERS&&!occ(x,y,capZ))moves.push({x:x,y:y,z:capZ,enPassant:{px:ep.px,py:ep.py,pz:ep.pz}});}}return moves;}
...
function _onBotWorkerResult(data) {
  ...
  if (!data || !gameStarted || reviewing) return;
